...
def watchlist_release_label(info):
    for key in ('date', 'premiered', 'aired'):
        value = info.get(key)
        if value:
            return value[:10] if len(value) >= 10 else str(value)
    year = info.get('year')
    if year:
        return str(year)
    return ''

def watchlist_release_iso(info):
    label = watchlist_release_label(info)
    if not label:
        return None
    if len(label) == 4 and label.isdigit():
        return "%s-01-01T00:00:00Z" % label
    return "%sT00:00:00Z" % label

def watchlist_apply_release_info(list_item, video_info_tag, info, fallback_label2=None):
    release_label = watchlist_release_label(info) or fallback_label2
    if not release_label:
        return
    list_item.setLabel2(release_label)
    if PLATFORM['kodi'] >= 20 and video_info_tag is not None:
        if len(release_label) >= 10:
            video_info_tag.setFirstAired(release_label[:10])
            video_info_tag.setPremiered(release_label[:10])
        elif release_label.isdigit() and len(release_label) == 4:
            video_info_tag.setYear(int(release_label))
        release_iso = watchlist_release_iso(info)
        if release_iso:
            list_item.setDateTime(release_iso)
    year = info.get("year")
    if PLATFORM['kodi'] >= 20 and video_info_tag is not None and year:
        try:
...
def run(url_suffix="", retry=0):
    ...
    raw_url = sys.argv[2]
    is_trakt_watchlist = "/trakt/watchlist" in raw_url
    ...
    if data["content_type"]:
        ...
        if is_trakt_watchlist:
            # %Y uses firstAired/year on the video tag; Estuary also reads ListItem.Year for sort 7/29
            xbmcplugin.addSortMethod(HANDLE, xbmcplugin.SORT_METHOD_UNSORTED, "%L", "%Y")
        else:
            xbmcplugin.addSortMethod(HANDLE, xbmcplugin.SORT_METHOD_UNSORTED)
        if content_type and not is_trakt_watchlist:
            try:
                # Movies
    ...
    for i, item in enumerate(data["items"]):
        ...
        if is_trakt_watchlist and item.get("info"):
            dateadded = item["info"].get("dateadded")
            if dateadded:
                listed_date = dateadded.split(" ")[0]
                if not item["label"].startswith(listed_date):
                    item["label"] = "%s | %s" % (listed_date, item["label"])
            release_label = watchlist_release_label(item["info"]) or item.get("label2")
            if release_label:
                item["label2"] = release_label
                item["info"]["premiered"] = release_label
                item["info"]["aired"] = release_label
        ...
        try:
            ...
            if item.get("info"):
                ...
                dateadded = item["info"].pop("dateadded", None)
                if PLATFORM['kodi'] < 20 and dateadded:
                    item["info"]["dateadded"] = dateadded
                listItem.setInfo("video", item["info"])
                if dateadded and PLATFORM['kodi'] >= 20:
                    video_info_tag.setDateAdded(dateadded)
                if is_trakt_watchlist:
                    watchlist_apply_release_info(
                        listItem,
                        video_info_tag if PLATFORM['kodi'] >= 20 else None,
                        item["info"],
                        item.get("label2"),
                    )
    ...
    if is_trakt_watchlist:
        try:
            xbmc.executebuiltin('Container.SetSortMethod(%d)' % xbmcplugin.SORT_METHOD_UNSORTED)
        except Exception as e:
            log.debug("Could not reset watchlist sort method: %s" % e)
