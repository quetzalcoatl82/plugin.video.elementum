...
def watchlist_listed_iso(dateadded):
    if not dateadded:
        return None
    if "T" in dateadded:
        return dateadded if dateadded.endswith("Z") else dateadded + "Z"
    return dateadded.replace(" ", "T") + "Z"

def watchlist_apply_listed_date(list_item, video_info_tag, dateadded):
    if not dateadded:
        return
    listed_date = dateadded.split(" ")[0]
    list_item.setLabel2(listed_date)
    if PLATFORM['kodi'] >= 20 and video_info_tag is not None:
        video_info_tag.setDateAdded(dateadded)
        video_info_tag.setFirstAired(listed_date)
        date_iso = watchlist_listed_iso(dateadded)
        if date_iso:
            list_item.setDateTime(date_iso)

def watchlist_strip_label_prefix(label):
    if " | " in label:
...
def run(url_suffix="", retry=0):
    ...
    for i, item in enumerate(data["items"]):
        ...
        if is_trakt_watchlist:
            item["label"] = watchlist_strip_label_prefix(item["label"])
            if item.get("info"):
                dateadded = item["info"].get("dateadded")
                if dateadded:
                    item["label2"] = dateadded.split(" ")[0]

        watchlist_dateadded = item.get("info", {}).get("dateadded") if is_trakt_watchlist else None
        ...
        try:
            ...
            if item.get("info"):
                ...
                if is_trakt_watchlist and dateadded:
                    watchlist_apply_listed_date(listItem, video_info_tag, dateadded)
                elif dateadded and PLATFORM['kodi'] >= 20:
                    video_info_tag.setDateAdded(dateadded)
            ...
            if is_trakt_watchlist and watchlist_dateadded:
                watchlist_apply_listed_date(listItem, video_info_tag, watchlist_dateadded)
        except Exception as e:
