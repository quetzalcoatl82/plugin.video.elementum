...
def run(url_suffix="", retry=0):
    ...
    if data["content_type"]:
        ...
        if is_trakt_watchlist:
            # %h = ListItem.Label2 (Kodi 21+); default %D uses datetime and shows epoch without setDateTime
            xbmcplugin.addSortMethod(HANDLE, xbmcplugin.SORT_METHOD_UNSORTED, "%L", "%h")
        else:
    ...
    for i, item in enumerate(data["items"]):
        ...
        try:
            ...
            if is_trakt_watchlist:
                ...
                release_label = watchlist_release_label(info) or item.get("label2")
                if release_label:
                    listItem.setLabel2(release_label)
                if PLATFORM['kodi'] >= 20:
                    release_iso = watchlist_release_iso(info)
                    if release_iso:
                        listItem.setDateTime(release_iso)
        except Exception as e:
